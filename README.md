...
### 5.1.2 `docker-compose.yml` 与 `.env` 的分工

本方案有两个配置文件，职责严格分开，不要混用：

| 文件 | 放什么 | 是否进 git |
|---|---|---|
| `docker-compose.yml` | **结构**：镜像、端口、挂载、变量名 | ✅ 会提交到公开仓库 |
| `.env` | **值**：模型地址、密钥、容量参数 | ❌ 已被 `.gitignore` 排除 |

**为什么必须这样分**：`docker-compose.yml` 是仓库的一部分，任何人都能看到。
密钥一旦写进那里，`git push` 就等于把密钥公开发布。放进不被跟踪的 `.env` 才能避免。

**工作机制**：Docker Compose 启动时会自动读取同目录的 `.env`，
用它替换 compose 文件里的 `${变量名}`。这里有两个很常见的误解：

- ❌ **`.env` 不会自动传进容器**。它只用于替换 compose 文件里的占位符；
  要让变量真正进入容器，必须在 compose 里显式写 `environment:`（本方案已经写好）。
- ❌ **compose 里没有 `${}` 时，`.env` 完全不起作用**。
  如果你把配置写成了字面值，那么改 `.env` 不会有任何效果，且不会有任何报错提示。

**改配置的正确姿势**：只编辑 `.env`，然后重建容器。

```bash
vi .env
docker compose up -d      # 重建容器使新值生效
```
...
dsh 用 `public, max-age=31536000, immutable` 发送客户端代码（因为其 URL 带构建版本号），
而本补丁改的是**内容**、URL 不变，`&rev=` 又不能改（dsh 会拒绝不匹配的版本号）。

后果：**在启用补丁之前访问过的浏览器，会把未打补丁的代码缓存一年**，
启用补丁后它不来取新的，看起来就像"补丁没生效、报错和之前一样"。

网关已对补丁后的响应改用 `no-cache` + ETag（浏览器必定重新校验，未变化时走 304
不产生实际流量），所以**新访问者不会再遇到这个问题**。但已经被缓存过的那批浏览器
需要一次性操作：

- 让同事按 **`Ctrl + Shift + R`**（硬刷新）一次即可，之后自动走 `no-cache`
- 或者用无痕窗口验证：无痕下正常 → 确认就是缓存问题

> 排查时容易误判：服务端日志显示 `patch applied`、`curl` 拉到的也是改写后的字节，
> 但浏览器仍用旧副本。**判断标准是浏览器实际执行了什么，不是服务端返回了什么。**
...
| 配置 | 归属 | 行为 |
|---|---|---|
| `settings.yaml`<br>（模型列表、默认模型、兼容开关） | **同事自己** | 首次登录时由 `.env` 生成一份初始值；之后同事在界面上的修改**永久保留** |
| `.credentials.yaml` 中被 `DSH_LLM_API_KEY` 指定的密钥 | **管理员** | 每次启动都从 `.env` 刷新，便于你集中轮换泄露的密钥 |
| 其他密钥（同事自己新增的 provider 所用的 key） | 同事自己 | 保留不动 |

**你想收回配置权**时，在 `.env` 里加一行：

```ini
DSH_CONFIG_OVERWRITE=1
```

这样每次启动都会用 `.env` 重新生成 `settings.yaml`，同事的本地修改会被覆盖，
界面上也就改不动了。**只想重置某一个人的配置**，删掉他的文件即可：
...
> ⚠️ 一个容易踩的坑：dsh 的配置写入器**会保留注释**，所以无法靠"注释还在不在"
> 判断文件是自动生成的还是被改过的。早期实现按这个判断去覆盖，结果同事在界面上
> 改的模型在实例回收后**静默丢失**（不报错、无提示）。现在改为一旦文件存在就不再覆盖。
> 回归测试见 `_probe/test-settings-persist.sh`。
...
每个用户是一个完整的 dsh 进程，加载了大量插件，因此单实例开销不小。
按 `100MB + 在线人数 × 600MB` 估算，默认 `GATEWAY_MAX_INSTANCES=10`
对应约 **6GB** 内存。服务器内存紧张时请调小该值——用户闲置后实例会被回收，
内存随之释放，下次登录再自动拉起。
